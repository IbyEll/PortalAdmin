...
function dbSeedStatePath() {
  return join(getPortalRoot(), "data", "utility-db-seed-state.json");
}
...
export async function startRepoServices(options = {}) {
  ...
  if (needsProductStack) {
    ...
    if (!startAll.ok) {
      state.running = false;
      state.error   = startAll.error ?? "start_ALL_Services fallito";
...
/**
 * Elenco servizi dev dal manifest con PID in ascolto sulle porte.
 */
...
/**
 * Stato file SQLite del product repo.
 */
...
export async function startSingleRepoService(serviceId) {
  ...
  if (PRODUCT_CORE_SERVICE_IDS.includes(serviceId)) {
    ...
    pushLogLine("system", `=== Avvio stack product (${serviceId}) via start_ALL_Services ===`);
