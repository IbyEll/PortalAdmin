 ...
 *
 * Archivio e rigenerazione piano Jira Working — HTML statico cruscotto.jira.working.html.
 *
 * Descrizione funzionale:
 *
 *   Perché esiste:
 *   - il working plan è una pagina HTML generata con sezioni sprint, note e insight embedded
 *   - serve versioning archivi e rigenerazione senza modificare sprint Jira (solo lettura API)
 *
 *   A cosa serve:
 *   - regenerateWorkingPlanHtml, archiveAndRegenerateWorkingPlan — patch marker HTML piano
 *   - buildWorkingPlanContext, renderWorkingPlanBody — corpo piano da backlog + insight
 *   - listWorkingPlanArchives, workingArchivePath — storico snapshot in cruscotto.database/jira.working
 *
 * Generalizzazione:
 *   No — CRITICAL_CHAIN, marker HTML e path archivio dedicati al working plan JLO in PortalAdmin.
 *
 * Input:
 *   - issues — backlog da fetchJiraBacklog (interno alle funzioni regenerate)
 *   - cruscotto.jira.working.html — template con marker WORKING-PLAN-GENERATED, KEY-TO-BLOCK, ecc.
 *   - JLO_WORKING_PLAN — blocchi sprint da cruscotto.jira.working.order.mjs
 *
 * Consumatori:
 *   - runner/cruscotto.server.mjs — regenerateWorkingPlanHtml, archiveAndRegenerateWorkingPlan, archivi
 *
 * Export principali:
 *   - regenerateWorkingPlanHtml, archiveAndRegenerateWorkingPlan — rigenerazione piano
 *   - buildWorkingPlanContext, renderWorkingPlanBody — render sezioni piano
 *   - listWorkingPlanArchives, workingArchivePath, saveOldAndRebuildWorking — gestione archivi
 */
...
/**
 * @param {import("./cruscotto.jira.working.order.mjs").SprintExecutionPhase[]} phases
