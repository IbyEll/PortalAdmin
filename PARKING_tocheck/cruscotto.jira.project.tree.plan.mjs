 ...
 *
 * Rigenerazione Project Tree — albero Jira + piano Working (JLO_WORKING_PLAN).
 *
 * Descrizione funzionale:
 *
 *   Perché esiste:
 *   - cruscotto.jira.project.tree.html embedda dati JS generati; serve sync con backlog live e piano MVP
 *   - mappa sprint Jira → blocchi tree allineati a working.order.mjs
 *
 *   A cosa serve:
 *   - buildProjectTreeNodes — struttura ad albero per UI project tree
 *   - regenerateProjectTreeHtml — patch marker HTML (PROJECT-TREE-DATA, sprint map, header meta)
 *
 * Generalizzazione:
 *   No — TREE_BLOCK_DEFS e syntheticChildren hardcoded per piano JLO; usa JLO_WORKING_PLAN condiviso.
 *
 * Input:
 *   - backlog — output fetchJiraBacklog
 *   - cruscotto.jira.project.tree.html — template HTML con marker sostituibili
 *
 * Consumatori:
 *   - runner/cruscotto.server.mjs — regenerateProjectTreeHtml da API admin
 *
 * Export principali:
 *   - regenerateProjectTreeHtml — entry rigenerazione file HTML
 *   - buildProjectTreeNodes, buildSprintNameToTreeBlockMap — dati tree
 *   - renderProjectTreeJs, renderSprintNameToTreeBlockJs, renderProjectTreeHeaderMeta — snippet embed
 */
