 ...
 *
 *   Perché esiste:
 *   - il contenuto Confluence della matrice pilastri va adattato al layout statico cruscotto (:3999)
 *   - centralizza path portal, nav pilastri e conversione markup Confluence → classi CSS portal
 *
 *   A cosa serve:
 *   - renderPortalPage, renderIndexPillarTable — shell HTML con nav e asset cruscotto
 *   - confluenceHtmlToPortal — normalizza panel/status Confluence
 *   - pillarPageUrl, buildNavItems — link tra pagine pillar-matrix
 *
 * Generalizzazione:
 *   No — base path /pillar-matrix e prefisso cruscotto.pillar. fissi per portal Admin JLO.
 *
 * Input:
 *   - —
 *
 * Consumatori:
 *   - cruscotto.jira.pillar.matrix.portal.generate.mjs — scrittura pagine statiche
 *   - cruscotto.jira.backlog.pillars.mjs — pillarPageUrl per link backlog
 *
 * Export principali:
 *   - renderPortalPage, renderIndexPillarTable, renderIndexPillarList — markup HTML
 *   - confluenceHtmlToPortal — trasformazione HTML Confluence
 *   - pillarPageUrl, pillarPageFilename, buildNavItems — routing file statici
 */

const PORTAL_BASE = "/pillar-matrix";
const PILLAR_PAGE_PREFIX = "cruscotto.pillar.";
 ...
 * @returns {string}
 */
export function pillarPageFilename(pillarId) {
  return `${PILLAR_PAGE_PREFIX}${pillarId}.html`;
}
 ...
 * @returns {string}
 */
export function pillarPageUrl(pillarId) {
  return `${PORTAL_BASE}/${pillarPageFilename(pillarId)}`;
}
...
export function renderPortalPage(opts) {
  const nav = opts.navItems.map((item) => {
    const href = pillarPageUrl(item.id);
...
export function renderIndexPillarList(pillars) {
  const items = pillars.map((pillar) => {
    return `<li><a href="${pillarPageUrl(pillar.id)}">${esc(pillar.pillar)}</a></li>`;
  }).join("\n");
...
export function renderIndexPillarTable(pillars, pillarBlocks) {
  const rows = pillars.map((pillar, idx) => {
    ...
    return [
      "<tr>"
    , `<td><a href="${pillarPageUrl(pillar.id)}">${esc(pillar.pillar)}</a></td>`
    , `<td>${stats.total}</td>`
