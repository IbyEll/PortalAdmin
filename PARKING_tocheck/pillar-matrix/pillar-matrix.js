 ...
 *
 * Matrice pilastri — companion delle pagine pillar-matrix/cruscotto.pillar.*.html.
 *
 * Descrizione funzionale:
 *
 *   Perché esiste:
 *   - le pagine pillar sono HTML statiche generate; interazioni albero/gruppi/colonne vivono in questo script
 *   - evita framework build per UX matrice (resize, collapse, persistenza localStorage)
 *
 *   A cosa serve:
 *   - colonne ridimensionabili, raggruppamento issue (tbody.matrix-issue-group), albero (matrix-tree)
 *   - toolbar espandi/collassa via window.JloExpandCollapseUi; nasconde UI rigenera legacy
 *
 * Generalizzazione:
 *   No — selettori e classi DOM specifici matrice pilastri JLO; HTML da cruscotto.jira.pillar.matrix.portal.mjs.
 *
 * Input:
 *   - DOM main table (.matrix-grouped, .matrix-tree) — struttura generata nelle HTML companion
 *   - window.location.pathname — chiavi localStorage per larghezze e stato collapse
 *   - window.JloExpandCollapseUi — bottoni bulk expand/collapse (expand-collapse-ui.js)
 *
 * Pagina HTML:
 *   - cruscotto.frontend/pillar-matrix/cruscotto.pillar.*.html — <script src="/pillar-matrix.js">
 *
 * Servito da:
 *   - runner/cruscotto.server.mjs — static root cruscotto.frontend/
 *
 * Asset correlati:
 *   - expand-collapse-ui.js — window.JloExpandCollapseUi
 *   - pillar-matrix/pillar-matrix.css — stili matrice
 *   - cruscotto.jira.pillar.matrix.portal.generate.mjs — rigenerazione HTML pillar (solo CLI)
 */
