 ...
 *
 * Consumatori: lib/discovery-services-repo.mjs, lib/repo-service-discovery.mjs
 *
 * Export principali:
...
try {
  ({ DISCOVERY_CONFIG_VALUES } = await import(`./config.discovery.${overlayName}.mjs`));
} catch (err) {
  const notFound = err instanceof Error && "code" in err && err.code === "ERR_MODULE_NOT_FOUND";
  const hint     = notFound ? ` Crea lib/config.discovery.${overlayName}.mjs.` : "";
