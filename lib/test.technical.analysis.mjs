 ...
 *
 * ************************************************************************************************************************
 *     Analisi report TestTecnici — aggregazione latest.json, cluster, export MD/HTML.
 * ************************************************************************************************************************
 *
 * Descrizione funzionale:
 *
 *   Perché esiste:
 *   - Il cruscotto deve trasformare latest.json testScript in insight senza duplicare logica
 *     nel server dashboard.
 *
 *   A cosa serve:
 *   - buildTestTecniciAnalysis, render MD/HTML, persistenza history, loadAndAnalyze.
 *
 * Generalizzazione:
 *   Si — raccomandazioni da project.config; overlay può re-export modulo proprio.
 *
 * Input:
 *   - LATEST_JSON — path report da lib/reporter.mjs
 *   - PRJ_NPM_SCOPE, PRJ_WEB_OPEN_URL — contesto raccomandazioni in analisi
 *
 * Consumatori:
 *   - lib/dashboard.project.mjs — fallback se manca test.technical.analysis overlay
 *   - PROJECT overlay test.technical.analysis — estensioni per product
 *
 * Export principali:
 *   - TECNICI_ANALYSIS_JSON, TECNICI_ANALYSIS_HTML, TECNICI_ANALYSIS_MD — path output
 *   - buildTestTecniciAnalysis, renderTestTecniciAnalysisMd, renderTestTecniciAnalysisHtml
 *   - writeTestTecniciAnalysisArtifacts, loadAndAnalyzeTestTecnici
 *
 * ------------------------------------------------------------------------------------------------------------------------
 */
...
export function buildTecniciAnalysisRecommendations(ctx) {
  ...
  const ws              = config.PRJ_DB_NPM_WORKSPACE?.trim();
  const testScript      = config.PRJ_TEST_SCRIPT;

  if (ws && clusters["401 Unauthorized / login seed"]) {
  ...
  if (ws && clusters["utente seed assente"]) {
    recommendations.push(
      `Eseguire \`npm run db:seed -w ${ws}\` o impostare TEST_FORGOT_EMAIL per test forgot-password.`
    );
  }

  if (ws && failedScripts.some((s) => s.includes("notifications/"))) {
    recommendations.push(
