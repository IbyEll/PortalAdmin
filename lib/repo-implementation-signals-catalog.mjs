/**
 * Aggiorna REPO_IMPLEMENTATION_SIGNALS in portal.config.mjs
...
const CONFIG_FILE    = join(PORTAL_ROOT, "portal.config.mjs");
...
function runGit(cmd, args, opts = {}) {
  return runGitIn(PRODUCT_ROOT, cmd, args, opts);
}
...
const GITHUB_PRODUCT_REPO_URL = "https://github.com/IbyEll/JustLastOne";
const GITHUB_PORTAL_REPO_URL  = "https://github.com/IbyEll/PortalAdmin";

/**
 * @param {string} key
 */
function gitRootForKey(key) {
  return String(key).toUpperCase().startsWith("ADMIN-") ? PORTAL_ROOT : PRODUCT_ROOT;
}
...
function gitRootForBranch(branch) {
  if (/(?:STORY|BUG|TODO)---ADMIN-\d+/i.test(branch) || /^ADMIN-\d+-/i.test(branch)) {
    return PORTAL_ROOT;
  }
...
function githubUrlForKey(key) {
  return gitRootForKey(key) === PORTAL_ROOT
    ? GITHUB_PORTAL_REPO_URL
...
function branchMapForAllTickets() {
  ...
  const map = new Map();
  const reTyped = /(?:STORY|BUG|TODO)---((?:JLO|ADMIN)-\d+)-/i;
  const reLegacyJlo = /^JLO-(\d+)-/i;
  const reLegacyAdmin = /^ADMIN-(\d+)-/i;

  for (const root of [PORTAL_ROOT, PRODUCT_ROOT]) {
    for (const line of runGitIn(root, "git", ["branch", "-a"], { allowFail: true }).split("\n")) {
      ...
      const legacyJlo = name.match(reLegacyJlo);
      const legacyAdmin = name.match(reLegacyAdmin);
      const key = typed?.[1]?.toUpperCase()
        ?? (legacyJlo ? `JLO-${legacyJlo[1]}` : null)
        ?? (legacyAdmin ? `ADMIN-${legacyAdmin[1]}` : null);
...
export function resolveTicketBranch(key) {
  ...
  const parsed = String(key).trim().toUpperCase().match(/^(JLO|ADMIN)-(\d+)$/);
  ...
  const root = project === "ADMIN" ? PORTAL_ROOT : PRODUCT_ROOT;
  /** @type {string[]} */
  const found = [];
...
export function listAllTicketBranchKeys() {
  /** @type {Set<string>} */
  const keys = new Set();
  const re = /(?:STORY|BUG|TODO)---((?:JLO|ADMIN)-\d+)-/i;
...
export function writeRepoImplementationSignals(signals, opts = {}) {
  const markerIdx = readFileSync(CONFIG_FILE, "utf8").indexOf(SIGNALS_MARKER);

  if (markerIdx < 0) {
    throw new Error("REPO_IMPLEMENTATION_SIGNALS non trovato in portal.config.mjs");
  }
  ...
  if (!opts.dryRun) {
    writeFileSync(CONFIG_FILE, next, "utf8");
  }
...
export function commitCatalogUpdate(key) {
  const rel = posix.normalize("portal.config.mjs").replace(/^\.\//, "");
  const porcelain = runGitIn(PORTAL_ROOT, "git", ["status", "--porcelain", rel], { allowFail: true });

  if (!porcelain) {
    return null;
  }

  runGitIn(PORTAL_ROOT, "git", ["add", rel]);
