/**
 * Catalogo testScript — discovery generica + policy da PROJECT_{PRJ_NAME}/test.catalog.{Nome}.mjs.
 *
 * Descrizione funzionale:
 *   Perché esiste: run-all, dashboard e meta tecnici condividono walk filesystem e policy skip
 *     senza duplicare logica per ogni overlay product.
 *   A cosa serve: path product repo, discoverTestScripts(); overlay opzionale espone solo
 *     BLOCKED_SCRIPTS, EXCLUDED_SCRIPTS, BLOCKED_REASONS.
 *
 * Consumatori: lib/test.run-all.mjs, cruscotto.server.mjs, cruscotto.testscript.manager.mjs, …
 *
 * Export principali: discoverTestScripts, BLOCKED_SCRIPTS, EXCLUDED_SCRIPTS, BLOCKED_REASONS,
 *   getRepoRoot, REPO_ROOT, TEST_SCRIPT_DIR, requireTestScriptDir
 */
