 ...
 *!                        , runner (start_ALL_Services, start_API_Portal), dashboard-server
 ...
 *!   * Product ha priorità; PortalAdmin come fallback (es. start_ALL_Services).
 *!   
 ...
 *!  _______________________________________________________________________________________________
 *!  
 *!  4 . Stack product avviato da ADMIN — web/api/auth nel manifest puntano a runner/start_ALL_Services.mjs
 ...
 *!          │
 *!          ▼
 *!  repo-service-discovery  ◄── dev-manifest (ADMIN)
 ...
 *     — ADMIN: dev-manifest PRJ_DEV_MANIFEST, runner (start_ALL_Services, API Portal),
 ...
 *   REPO_EXTRAS_ALL, PRODUCT_REPO_EXTRAS — id extra per tab Utility
 */
...
// --- costanti export — stack Utility cruscotto ---
...
/** Stack product in tabella Utility (solo core) */
...
/**
 * Manifest JSON: product repo locale, poi loadDevManifest se repo = product configurato.
 *
 * @param {string} repoRoot
 */
async function resolveManifestForRepo(repoRoot) {
  // 1. dev-manifest.json nel repo scansionato (root o cruscotto/)
  const local =
  ...
  // 2. Product repo senza manifest locale — usa manifest PortalAdmin (PRJ_DEV_MANIFEST)
...
function enrichManifestService(repoRoot, svc) {
  ...
  if (id === "api-portal" && PORTAL_SERVICE_IDS.has("api-portal")) {
