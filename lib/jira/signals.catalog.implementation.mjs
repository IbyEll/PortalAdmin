/**
 * Catalogo segnali implementazione repo — path e test per ticket Jira del progetto attivo (PRJ_NAME).
 *
 * Descrizione funzionale:
 *
 *   Perché esiste:
 *   - collegare ogni key Jira ai path sorgente reali senza duplicare git/scan in ogni script
 *   - policy path significativi centralizzata in jira.config (REPO_SIGNALS_CATALOG_CONFIG)
 *
 *   A cosa serve:
 *   - deriva paths, tests e label da branch/commit git sul repo del progetto attivo
 ...
 *
 * Consumatori:
 *   - scripts/close-story.mjs — ensureRepoImplementationSignal, commitCatalogUpdate
 *   - scripts/sync-repo-catalog.mjs — signalKeyExistsInFile, listAllTicketBranchKeys
 *   - lib/jira/jira.backlog.insights.mjs — isMeaningfulCitationPath
 *   - scripts/generate-confluence-pillar-matrix.mjs — resolveTicketGitEvidence
 *
 * Export principali:
 ...
 *   - writeRepoImplementationSignals, replace/remove/appendSignalInInsightsFile — edit config
 *   - commitCatalogUpdate — commit catalogo su PortalAdmin dopo chiudi ticket
 *
 * Env: PRJ_NAME, PRODUCT_REPO_PATH (via portal.paths.resolver)
 * Config: REPO_SIGNALS_CATALOG_CONFIG + overlay PRJ_* (config.project)
 */
...
} from "../admin/config.project.mjs";
import { scanRepoJiraReferences } from "../function.repo.jira.refs.mjs";
import { getPortalRoot, getProductRepoPath } from "../portal.paths.resolver.mjs";
import {
...
// --- portal.config.mjs — serializzazione e rewrite file segnali (product / portal) ---
/**
 ...
 * Usato da close-story.mjs dopo aggiornamento PRODUCT_REPO_SIGNALS / PORTAL_ADMIN_REPO_SIGNALS.
 *
 * @param {string} key
