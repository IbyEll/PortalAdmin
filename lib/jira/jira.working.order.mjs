/**
 * Ordine di sviluppo JLO — allineato a cruscotto/jira.working.html
 * e scripts/create-jlo-sprints.mjs.
 *
 * Regole colonna «Ordine Sviluppo» (backlog cruscotto):
 *
 * 1. Epic nel piano MVP → `{sprint}` (solo numero sprint, es. `2`)
 * 2. Story/Bug/Todo nel piano MVP → `{sprint}.{seq}` (seq = posizione nella lista sprint del piano, es. `2.5`)
 *    Vale sempre, anche se l'issue è figlia di un epic in un altro sprint.
 * 3. Story/Bug/Todo NON nel piano, figlia di epic nel piano → `{sprint}.{seq}`
 *    seq = maxSeq(sprint) + indice locale tra le issue sorelle sotto quell'epic (es. `2.7`)
 * 4. Subtask → `{storyOrder}.{sub}` dove storyOrder è l'ordine della parent story-like (es. `2.5.1`)
 * 5. Fuori piano → `—` (null)
 * 6. Sprint 6 — issue non nel piano e non `[Obsoleto]` sul board sprint 6 → `{sprint}.{maxPlanSeq+1}` … (es. `6.33`, `6.34`); subtask → `6.33.1`
 * 7. Sprint 6 — obsolete (`JLO_SPRINT_6_OBSOLETE` o summary `[Obsoleto]`) → `—` (null)
 *
 * `devSprintName` — nome sprint dal piano (`JLO_WORKING_PLAN[].name`), es. `Sprint 2 — Fase 0`.
 * Story-like (Story/Bug/Todo) nel piano o figlie di epic nel piano ereditano il nome; subtask ereditano dal numero sprint.
 * La colonna «Ordine Sviluppo» nel cruscotto nasconde il valore per le righe epic (UI only).
 */
