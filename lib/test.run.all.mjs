...
/**
 * ------------------------------------------------------------------------------------------------------------------------
 * ** SCRIPT ENTRYPOINT ** -- commentato il: 2026-06-23 20:42
 * ------------------------------------------------------------------------------------------------------------------------
 ...
 *
 *   Perché esiste:
 *   - PortalAdmin serve un unico entrypoint per la suite del product repo attivo senza script
 *     duplicati per overlay.
 *
 *   A cosa serve:
 *   - Catalogo overlay, check auth/api/web, reset DB seed, run sequenziale, merge report latest.
 *
 * Generalizzazione:
 *   Si — discovery e fixture da PRJ_NAME; probe stack da env e project.config.
 *
 * Input:
 *   - PRJ_NAME, PRODUCT_REPO_PATH — product attivo
 *   - argv --list, --script, --suite, --test, --no-html
 *   - AUTH_HEALTH_URL, API_HEALTH_URL, WEB_OPEN_URL — override probe stack
 *   - DATABASE_URL — override SQLite; default via test.match-fixtures overlay
 *   - JLO_TEST_ONLY, JLO_TEST_CHAIN — propagati al child su --test
 *
 * Uso:
 *   - node lib/test.run.all.mjs
 *   - node lib/test.run.all.mjs --list
 *   - node lib/test.run.all.mjs --script auth/test-login.mjs --test "login seed host"
 *
 * Flag CLI:
 ...
 *   --test       singolo test case (--script obbligatorio)
 *
 * Prerequisiti:
 *   - testScript nel product repo; auth e api raggiungibili su /health
 *
 * Exit code:
 *   0 — tutti gli script passati
 *   1 — almeno un fallimento o prerequisito mancante
 *
 * ------------------------------------------------------------------------------------------------------------------------
 */
...
import { checkProjectServices } from "../cruscotto.frontend/cruscotto.runner.stack.probe.mjs";
import {
...
import { projectHasProductDatabase } from "./project.config.mjs";
...
async function main() {
  ...
  // 3. Prerequisiti stack + pre-flight DB seed host (solo se product ha database)
