...
export function resolveProjectOverlayName() {
  ...
  if (!fromEnv) {
    throw new Error(
      [
        "config.project — PRJ_NAME obbligatorio."
...
try {
  ({ PROJECT_CONFIG_VALUES } = await import(`../../PROJECT_${overlayName}/config.project.${overlayName}.mjs`));
} catch (err) {
  const notFound = err instanceof Error && "code" in err && err.code === "ERR_MODULE_NOT_FOUND";
  const hint     = notFound ? ` Crea PROJECT_${overlayName}/config.project.${overlayName}.mjs.` : "";
