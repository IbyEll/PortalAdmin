...
const DATA_DIR          = join(PORTAL_ROOT, "data");
...
function requiredOverlayFiles(overlay) {
  return [
    `PROJECT_${overlay}/config.project.${overlay}.mjs`
...
/**
 * @param {string} overlay
 * @returns {Promise<import("./config.project.mjs").ProjectConfig | null>}
 */
export async function loadOverlayConfig(overlay) {
  const file = join(PORTAL_ROOT, `PROJECT_${overlay}`, `config.project.${overlay}.mjs`);
...
/**
 * @param {string} overlay
 * @param {import("./config.project.mjs").ProjectConfig} [config]
...
function readInstancesRegistry() {
  ...
  if (existsSync(LEGACY_INSTANCE)) {
    try {
      const legacy = /** @type {PortalInstanceState} */ (
        JSON.parse(readFileSync(LEGACY_INSTANCE, "utf8"))
      );
...
export async function activatePortalInstance(overlay, opts = {}) {
  ...
  if (!config) {
    throw new Error(`Overlay "${overlay}" non trovato (config.project mancante)`);
  }
  ...
  const script = join(PORTAL_ROOT, "scripts", "portal-prepare-instance.mjs");
