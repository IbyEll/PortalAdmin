...
if (opts.help) {
  console.log(`Uso: node runner/cruscotto.process.start.all.services.mjs [opzioni]
...
Singoli servizi:
  ...
  node runner/cruscotto.process.start.service.mjs api
  node runner/cruscotto.process.start.service.mjs web
`);
  process.exit(0);
}
...
if (prepareOnly) {
  ...
  console.log("Avvio stack: node runner/cruscotto.process.start.all.services.mjs --no-build");
  process.exit(0);
}
...
console.log("Kill da Process cruscotto o: node runner/cruscotto.process.stop.all.services.mjs");
