 ...
 *   Perché esiste: cruscotto.runner.stack.base resta generico; path clean, build order, stack dev
 ...
 *
 * Overlay attivo: project.config.{name}.mjs e runner.config.{name}.mjs
 *   con name = resolveProjectO verlayName() (env PRJ_NAME obbligatorio).
 */
...
/**
 * Nome file overlay `runner.config.{name}.mjs` — allineato a project.config.
 *
 * @returns {string}
 */
