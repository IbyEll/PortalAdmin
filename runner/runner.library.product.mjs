/**
 * Runner product — orchestrazione stack dev da runner.get_config + config.project.
 *
 * Descrizione funzionale:
 *   Perché esiste: policy di build, prepare nest/web, avvio servizi e Prisma
 *     dipendono dal monorepo product attivo — non vanno in runner.library generico.
 ...
 *
 * Export principali:
 *   getDevService, getDevServices, getStackRunnerEntries — metadati stack da runner.get_config
 ...
 *   root, run, runTurbo — re-export da runner.library
 *
 * Variabili d'ambiente:
 *   PRODUCT_REPO_PATH — root monorepo (via runner.library / portal-paths)
 *   {PRJ_JIRA_PREFIX}_FRIEND_BOT — es. JLO_FRIEND_BOT=0 disabilita daemon opzionali
 *   NODE_ENV=production — skip daemon dev
 *
 * Dipendenze: runner/runner.library.mjs, runner/runner.get_config.mjs, lib/config.project.mjs,
...
/**
 * @param {string} id
 * @returns {import("./runner.get_config.mjs").ResolvedDevService}
 */
...
/** Path .env e clean path da runner.get_config (wrapper su runner.library). */
export function ensureProductEnvFiles() {
  ensureEnvFiles(resolveEnvFiles());
}

/** Cleanup artefatti da runner.get_config.cleanPaths se --cleanup. */
...
/** Build workspace in ordine da runner.get_config.workspaceBuildOrder. */
...
export function prepareProductRepo(options = {}) {
  ...
  // 1. prerequisiti comuni — node_modules + .env da runner.get_config
  ensureNodeModules();
...
/**
 * @param {string} serviceId
 * @param {import("./runner.get_config.mjs").ResolvedDevService} cfg
...
function printDevServiceHelp(serviceId, cfg, scriptRel) {
  ...
  console.log(`Uso: node runner/start_DEV_Service.mjs <${ids}> [opzioni]
...
/**
 * Avvio singolo servizio dev (auth | api | web | … da runner.get_config).
 *
 * @param {string} serviceId
 * @param {string[]} argv
...
export function runDevServiceStart(serviceId, argv, meta = {}) {
  ...
  const scriptRel  = meta.scriptRel ?? `start_DEV_Service.mjs ${serviceId}`;
 ...
 * @param {string[]} argv
 */
/** Entrypoint CLI per start_DEV_Service.mjs — parse id e delega a runDevServiceStart. */
export function runDevServiceStartCli(argv) {
  ...
  if (!serviceId) {
    ...
    console.error(`Uso: node runner/start_DEV_Service.mjs <${ids}> [opzioni]`);
    console.error("     node runner/start_DEV_Service.mjs --service <id> [opzioni]");
    process.exit(1);
  }
...
export function startDevStack(options = {}) {
  ...
  if (!runnerCfg.useTurbo) {
    console.error("startDevStack — useTurbo disabilitato in runner.get_config; avvia i servizi singolarmente.");
    process.exit(1);
  }
