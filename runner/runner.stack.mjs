/**
 * Runner stack — orchestrazione dev da runner.config.stack + config.project.
 *
 * Descrizione funzionale:
 *   Perché esiste: policy di build, prepare nest/web, avvio servizi e Prisma
 *     dipendono dal monorepo product attivo — non vanno in runner.stack.base generico.
 ...
 *   root, run, runTurbo — re-export da runner.stack.base
 *
 * Variabili d'ambiente:
 *   PRODUCT_REPO_PATH — root monorepo (via runner.stack.base / portal-paths)
 *   {PRJ_JIRA_PREFIX}_FRIEND_BOT — es. JLO_FRIEND_BOT=0 disabilita daemon opzionali
 *   NODE_ENV=production — skip daemon dev
 *
 * Dipendenze: runner/runner.stack.base.mjs, runner/runner.config.stack.mjs, lib/config.project.mjs,
...
/**
 * @param {string} id
 * @returns {import("./runner.config.stack.mjs").ResolvedDevService}
 */
...
/** Path .env e clean path da runner.config.stack (wrapper su runner.stack.base). */
...
/**
 * @param {string} serviceId
 * @param {import("./runner.config.stack.mjs").ResolvedDevService} cfg
