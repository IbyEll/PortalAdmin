 ...
 *   Perché esiste: PortalAdmin serve un unico entrypoint per la suite JLO nel product repo
 *     (dashboard POST /api/run, smoke CI, cruscotto) senza shell ad hoc per script.
 *   A cosa serve: discovery catalogo, check auth/api/web, reset stato seed Prisma, run
 *     sequenziale con skip policy, merge report parziali e scrittura data/reports/latest.json.
 *
 * Uso:
 *   node runner/JustLastOne___run-all.mjs
 *   node runner/JustLastOne___run-all.mjs --list
 *   node runner/JustLastOne___run-all.mjs --no-html
 *   node runner/JustLastOne___run-all.mjs --script auth/test-login.mjs
 *   node runner/JustLastOne___run-all.mjs --suite auth
 *   node runner/JustLastOne___run-all.mjs --script auth/test-login.mjs --test "login seed host"
 *
 * Flag CLI:
 ...
 *   AUTH_URL, API_URL, WEB_BASE  endpoint stack (da .env)
 *   DATABASE_URL                 override SQLite; default via match-fixtures
 *   JLO_TEST_ONLY, JLO_TEST_CHAIN  propagati al child in modalità --test
 *
 * Prerequisiti:
 *   testScript/ nel product repo; auth :4001 e api :4000 raggiungibili (/health)
 */
...
} from "../lib/JustLastOne___prj_testScript_catalog.mjs";
import {
...
import { resetHostTestState, setupDefaultDatabaseUrl } from "../lib/JustLastOne___match-fixtures.mjs";
 ...
 *
 * @param {import("../lib/JustLastOne___prj_testScript_catalog.mjs").ScriptEntry} entry
