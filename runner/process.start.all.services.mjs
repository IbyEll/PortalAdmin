#!/usr/bin/env node
/**
 * process.start.all.services — avvio stack product (Auth + API + Web)
 */
...
if (opts.help) {
  console.log(`Uso: node runner/process.start.all.services.mjs [opzioni]
...
Singoli servizi:
  node runner/process.start.service.mjs auth
  node runner/process.start.service.mjs api
  node runner/process.start.service.mjs web
`);
  process.exit(0);
}
...
console.log("process.start.all.services — stack Auth + API + Web");
...
if (prepareOnly) {
  ...
  console.log("Avvio stack: node runner/process.start.all.services.mjs --no-build");
  process.exit(0);
}
...
console.log("Kill da Utility cruscotto o: node runner/process.stop.all.services.mjs");
