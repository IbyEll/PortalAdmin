 ...
 *
 * Dipendenze: lib/discovery.services.repo.mjs, runner/kill-dev-ports.mjs,
 *   runner/JustLastOne___kill-product-stack.mjs, lib/cruscotto-db/script_seed
 *
...
, PRODUCT_NEST_PORTS
} from "./JustLastOne___kill-product-stack.mjs";
import {
...
, shortenNodeCommand
} from "./list-project-node-processes.mjs";
...
const START_ALL_SERVICES_SCRIPT = "runner/cruscotto.process.start.all.services.mjs";

const PRODUCT_CORE_SERVICE_IDS = ["web", "api", "auth"];

// --- persistenza stato seed DB Process (data/process-db-seed-state.json) ---
/** @returns {string} */
function dbSeedStatePath() {
  return join(getPortalRoot(), "data", "process-db-seed-state.json");
}
...
function readDbSeedState() {
  ...
  const legacyFile = join(getPortalRoot(), "data", "utility-db-seed-state.json");
...
/**
 * Servizi e piano avvio dal dev-manifest product (+ extras opzionali).
 *
 * @param {{
 *   extras?: string[]
 *   withPortal?: boolean
...
export async function listDevStackProcesses() {
  ...
  for (const svc of discovery.services) {
    if (typeof svc.port === "number") {
      for (const pid of findListeningPids(svc.port)) {
