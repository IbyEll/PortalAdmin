/**
 * Aggiornamento mirato matrice pilastri cruscotto per ticket JLO/ADMIN (chiudi story --pillar).
 */

import { execFileSync } from "node:child_process";

import { findPillarsForKey, generatePillarMatrixHtml } from "./confluence.pillar.matrix.generate.mjs";
...
const PORTAL_DIR = "cruscotto.frontend/pillar-matrix";
...
function normalizeTicketKey(ticketKey) {
  return String(ticketKey).trim().toUpperCase().match(/(?:JLO|ADMIN)-\d+/)?.[0] ?? null;
}
