...
, {
  ...
  , area        : "Admin"
  , anchorKeys  : ["JLO-849", "JLO-6", "JLO-330", "ADMIN-88", "ADMIN-73"]
  , docExcerpt  : [
...
/**
 * @param {{ devSprintName?: string | null, jiraSprints?: Array<{ name: string, state: string }> } | null} row
 * @returns {string | null}
 */
function resolveIssueSprintName(row) {
  if (!row) {
    return null;
  }
  ...
  const closed = sprints.filter((sprint) => sprint.state === "closed").pop();
...
export function findPillarsForKey(ticketKey, pillars, issues) {
  const key = String(ticketKey).trim().toUpperCase().match(/(?:JLO|ADMIN)-\d+/)?.[0];

  if (!key) {
    return [];
  }

  const matched = pillars.filter((pillar) => keysForPillar(pillar, issues).includes(key));
  ...
  if (key.startsWith("ADMIN-")) {
    const adminDev = pillars.find((pillar) => pillar.id === "admin-dev");

    return adminDev ? [adminDev] : [];
  }
...
function buildMatrixIssueRow(key, byKey, repoRefs, gitCache, tree) {
  ...
  const obsolete = row?.isObsolete
    ? ` <span data-type="status" data-color="neutral">obsoleto</span>`
    : "";
  const type = row?.type ?? "—";
...
async function buildMatrix(opts = {}) {
  ...
  applyWorkingPlanToIssues(backlog.issues);
  ...
  const productRepo = getProductRepoPath({ required: false });
  ...
  , `<p><strong>Backlog Jira:</strong> ${backlog.total} issue (tutti gli stati) · generato <time datetime="${backlog.fetchedAt.slice(0, 10)}">${backlog.fetchedAt.slice(0, 10)}</time>${productRepo ? ` · scan repo <code>${esc(productRepo)}</code>` : ""}</p>`
