 ...
 *
 *   Perché esiste:
 *   - La HOME PortalAdmin deve attivare un product overlay senza editare manualmente .env.
 *
 *   A cosa serve:
 *   - Elenco PROJECT overlay, persistenza multi-istanza, patch .env e spawn prepare.
 *
 * Generalizzazione:
 *   Si — ogni overlay ha porta dashboard dedicata e record in portal-instances.json.
 *
 * Input:
 *   - PRJ_NAME — overlay da attivare (body API POST /api/portal/instance)
 *   - PRODUCT_REPO_PATH — path product scritto in .env all'attivazione
 *
 * Consumatori:
 *   - admin.portal/portal.home.server.mjs — API istanza e lista progetti
 *   - admin.portal/portal.instance.prepare.mjs — prepare post-attivazione
 *
 * Export principali:
 *   - listAvailableProjects, getPortalInstance, getPortalInstances
 *   - activatePortalInstance — patch .env e avvio prepare
...
/** @type {Map<string, import("node:child_process").ChildProcess>} */
const dashboardChildren = new Map();
 ...
 * @param {string} overlay
 */
function appendDashboardLog(logChunk, overlay) {
  const current = readInstanceForOverlay(overlay);
...
/**
 * Avvia cruscotto.server.mjs con log nella console HOME (stdio piped, nessuna finestra cmd).
 *
 * @param {string} overlay
 * @returns {Promise<{
 *   ok             : boolean
 ...
 * }>}
 */
export async function startPortalDashboard(overlay) {
  const instance = readInstanceForOverlay(overlay);

  if (!instance) {
    throw new Error(`Istanza non trovata per ${overlay} — esegui Istanzia`);
  }

  if (instance.prepare.status !== "done") {
    throw new Error(`prepare non completato per ${overlay}`);
  }
  ...
  const runningChild = dashboardChildren.get(overlay);
  ...
  if (runningChild && runningChild.exitCode === null) {
    ...
    , port
    , overlay
    , pid       : runningChild.pid ?? undefined
    };
  }

  /** @type {NonNullable<PortalInstanceState["dashboard"]>} */
  const dashboardState = {
    status     : "starting"
  , startedAt  : new Date().toISOString()
  , finishedAt : null
  , exitCode   : null
  , logTail    : `[cruscotto] avvio ${overlay} su :${port}…\n`
  , pid        : null
  };

  instance.dashboard = dashboardState;
  upsertInstanceState(instance);
  ...
  dashboardChildren.set(overlay, child);

  const current = readInstanceForOverlay(overlay) ?? instance;

  if (current.dashboard) {
    current.dashboard.pid = child.pid ?? null;
    upsertInstanceState(current);
  }

  child.stdout?.on("data", (chunk) => {
    appendDashboardLog(chunk.toString(), overlay);
  });
  ...
  child.on("close", (code) => {
    const inst = readInstanceForOverlay(overlay);

    if (!inst?.dashboard) {
      dashboardChildren.delete(overlay);
      return;
    }

    inst.dashboard.finishedAt = new Date().toISOString();
    inst.dashboard.exitCode   = code ?? 1;

    if (code === 0) {
      inst.dashboard.status = "stopped";
    } else if (!isPortListening(port)) {
      inst.dashboard.status = "error";
      appendDashboardLog(`\n[cruscotto] processo terminato (exit ${code ?? "?"})`, overlay);
    }

    upsertInstanceState(inst);
    dashboardChildren.delete(overlay);
  });
...
/**
 * @param {string} [overlay]
 * @returns {PortalInstanceState["dashboard"] | null}
 */
export function getDashboardStatus(overlay) {
  if (!overlay) {
    const instances = readAllInstances();

    return instances.at(-1)?.dashboard ?? null;
  }
  ...
  const child = dashboardChildren.get(overlay);
  ...
  if (child && child.exitCode === null) {
    if (!instance.dashboard) {
      ...
      instance.dashboard.status = "starting";
      instance.dashboard.pid  = child.pid ?? instance.dashboard.pid;
    }
  } else if (isPortListening(instance.dashboardPort)) {
    if (!instance.dashboard) {
      instance.dashboard = {
        status     : "running"
      , startedAt  : null
    ...
    } else if (instance.dashboard.status !== "error") {
      instance.dashboard.status = "running";
    }
  }

  return instance.dashboard ?? null;
}

/**
 * Termina cruscotto sulla porta overlay e rimuove l'istanza dal registry HOME.
 *
 * @param {string} overlay
 * @returns {Promise<{
 *   overlay: string
 ...
 * }>}
 */
export async function deactivatePortalInstance(overlay) {
  const instance = readInstanceForOverlay(overlay);

  if (!instance) {
    throw new Error(`Istanza non trovata per ${overlay}`);
  }

  const port       = instance.dashboardPort;
  const killResult = killDashboardOnPort(port);
  const child      = prepareChildren.get(overlay);
  const dashChild  = dashboardChildren.get(overlay);
  ...
  if (dashChild && dashChild.exitCode === null) {
  ...
  delete registry.instances[overlay];
  writeInstancesRegistry(registry);
