/**
 * Analisi strutturata dell'ultimo report test (TestTecnici).
 */
...
import { historyStamp, LATEST_JSON, REPORTS_DIR } from "../lib/reporter.mjs";
 ...
 */
export function buildTestTecniciAnalysis(report) {
  ...
  if (clusters["401 Unauthorized / login seed"]) {
    recommendations.push(
      "Allineare seed e credenziali test: `npm run db:seed -w @justlastone/database` e verificare password in testScript/lib/http.mjs."
    );
  }

  if (report.services && /** @type {{ web?: boolean }} */ (report.services).web === false) {
    recommendations.push("Avviare Next.js (:3000) per eseguire gli script web attualmente skipped.");
  }

  if (clusters["utente seed assente"]) {
    recommendations.push("Eseguire db:seed o impostare TEST_FORGOT_EMAIL per test forgot-password.");
  }

  if (failedScripts.some((s) => s.startsWith("notifications/"))) {
    recommendations.push("Dopo seed: `npm run db:push -w @justlastone/database` e rilanciare notifications/*.mjs.");
  }
...
export function renderTestTecniciAnalysisMarkdown(analysis) {
  ...
  lines.push("# Analisi TestTecnici — JustLastOne");
  lines.push("");
  ...
  if (s.services && typeof s.services === "object") {
    const svc = /** @type {{ auth?: boolean, api?: boolean, web?: boolean }} */ (s.services);
    lines.push(`- **Servizi:** auth ${svc.auth ? "✅" : "❌"} · api ${svc.api ? "✅" : "❌"} · web ${svc.web ? "✅" : "❌"}`);
  }
...
export function renderTestTecniciAnalysisHtml(analysis) {
  ...
  const services = s.services && typeof s.services === "object"
    ? /** @type {{ auth?: boolean, api?: boolean, web?: boolean }} */ (s.services)
    : null;
...
<body>
  <div class="page">
    ...
    ${services
      ? `<p class="meta">Servizi: auth ${services.auth ? "✅" : "❌"} · api ${services.api ? "✅" : "❌"} · web ${services.web ? "✅" : "❌"}</p>`
      : ""}
    <h2>Per suite</h2>
    <table>
 ...
 * @param {Record<string, unknown>} report
 */
export async function writeTestTecniciAnalysisReport(report) {
  const analysis = buildTestTecniciAnalysis(report);
