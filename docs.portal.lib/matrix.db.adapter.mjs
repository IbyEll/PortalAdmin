 ...
 */
export async function enrichMatrixSectionsFromJira(sections, portalRoot) {
  ...
  await enrichMatrixRowsWithIssueRefinement(rows, portalRoot);
}
...
export async function buildUnifiedMatrixSectionsWithDb(portalRoot, opts = {}) {
  ...
  if (useDbSections && cruscottoDbFileExists()) {
    ...
    if (fromDb.length > 0) {
      await enrichMatrixSectionsFromJira(fromDb, portalRoot);
  ...
  await enrichMatrixSectionsFromJira(built.sections, portalRoot);
