/**
 * Scansione avanzamento PortalAdmin — finding architettura, gap, bug, deprecation, feature.
 *
 * Consumatori:
 *   - docs.portal/matrix.portal.gap.analysis.procedure.mjs
 *   - docs.portal.lib/docs.portal.refresh.mjs
 */

import { existsSync, readFileSync, readdirSync, statSync } from "node:fs";
import { join } from "node:path";

import { enrichFindingsWithIssueRefinement, appendPersistedResolvedFindings } from "./matrix.finding.issues.mjs";
import { enrichFindingsWithProject } from "./matrix.finding.project.mjs";
import { analyzeRepository, grepRepoFiles } from "./docs.portal.analysis.mjs";
...
/**
 * File .mjs fuori PARKING con import o re-export verso PARKING_tocheck.
 *
 * @param {string} portalRoot
 * @param {string} [rootRel]
 * @returns {string[]}
 */
export function grepLiveParkingImports(portalRoot, rootRel = ".") {
  /** @type {string[]} */
  const hits = [];
  ...
  const walk = (dirRel) => {
    ...
    for (const name of readdirSync(abs)) {
      ...
      if (LIVE_PARKING_IMPORT_RE.test(readTextAt(portalRoot, rel))) {
        hits.push(rel);
      }
    }
  };
...
export async function analyzePortalAdvancement(portalRoot) {
  ...
  const pkg      = readTextAt(portalRoot, "package.json");
  ...
  , { id: "arch-docs-portal", title: "Documenti HTML + chrome su HOME :3990", paths: ["docs.portal/", "admin.portal/portal.home.html"], ok: base.checks.documentiTab?.ok && base.checks.docsChrome?.ok }
  ...
  const parkingImports = grepLiveParkingImports(portalRoot, ".");
  ...
  const publishJs = readTextAt(portalRoot, "admin.script.standalone/confluence.pillar.matrix.publish.mjs");
  ...
  if (readTextAt(portalRoot, ".github/workflows/portal-smoke.yml").includes("portal-paths.mjs") && !existsSync(join(portalRoot, "admin.portal.lib/portal-paths.mjs"))) {
    add({
  ...
  const runnerHits = grepRepoFiles(portalRoot, String.raw`runner/cruscotto\.server`);
  ...
  if (runnerHits.length > 0) {
    ...
    , severity : "P2"
    , title    : "Commenti/doc citano runner/cruscotto.server.mjs (legacy)"
    , detail   : `${runnerHits.length} file attivi con path legacy; canonico cruscotto.frontend/cruscotto.server.mjs.`
    , paths    : runnerHits.slice(0, 12)
    , status   : "open"
    });
  }
  ...
  const shimPortal = grepRepoFiles(portalRoot, String.raw`admin\.portal\.lib/portal-paths\.mjs`);
  ...
  , detail   : "docs.portal.lib — verifica automatica, sezione DOCS-AUTO-ADDITIONS, stelline su delta."
  , paths    : ["docs.portal.lib/docs.portal.mjs", "docs.portal.lib/docs.portal.analysis.mjs", "docs.portal/utility.toolbar.document.js"]
  , status   : "done"
  });
  ...
  const workflowYaml   = readTextAt(portalRoot, ".github/workflows/portal-smoke.yml");
  const ciAdminOverlay = /smoke-admin-overlay/.test(workflowYaml)
    && /PRJ_NAME:\s*AdminDashBoard/.test(workflowYaml)
    && /test:home|smoke-home/.test(`${pkg}\n${workflowYaml}`)
    && existsSync(join(portalRoot, "test.smoke/smoke-home.mjs"));
  ...
  , detail   : ciAdminOverlay
      ? "Job smoke-admin-overlay in portal-smoke.yml con test:home su :3990."
      : "portal-smoke.yml copre solo JustLastOne; nessuno smoke HOME :3990."
  , paths    : [".github/workflows/portal-smoke.yml", "test.smoke/smoke-home.mjs"]
  , status   : ciAdminOverlay ? "done" : "open"
  });
  ...
  appendPersistedResolvedFindings(findings);
