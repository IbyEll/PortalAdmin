 ...
 *
 *   Perché esiste:
 *   - Le pagine docs hanno titoli diversi; la sezione auto Aggiorna deve mostrare solo controlli
 *     pertinenti all'argomento del documento.
 *
 *   A cosa serve:
 *   - Estrae il titolo da HTML, deduce topic (audit, jira, overlay, …) e filtra righe tabella
 *     architettura in docs.portal.refresh.mjs.
 *
 * Generalizzazione:
 *   Si — mapping topic e righe ARCHITECTURE_STATUS_ROWS riusabile su qualsiasi pagina docs.portal.
 *
 * Input (obbligatorio se Si; se No scrivere «Input: —»):
 *   - html — contenuto pagina per extractPageTitle
 *   - title — stringa titolo per inferPageScope
 *   - pageTopics, rowTopics — array stringhe per rowMatchesPageTopics
 *
 * Consumatori:
 *   - docs.portal.lib/docs.portal.refresh.mjs — inferPageScope, ARCHITECTURE_STATUS_ROWS
 *
 * Export principali:
 *   - extractPageTitle, inferPageScope — titolo e ambito dedotto
 *   - ARCHITECTURE_STATUS_ROWS, rowMatchesPageTopics — filtro righe architettura
 *
 * ------------------------------------------------------------------------------------------------------------------------
 */
