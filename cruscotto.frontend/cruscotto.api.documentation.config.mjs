...
import { getProductRepoPath, getPortalRoot } from "../admin.portal.lib/portal.paths.resolver.mjs";
...
/**
 * Risolve product.manifest: product repo prima, poi fallback PortalAdmin (PRJ_PRODUCT_MANIFEST).
 *
 * @param {string} productRoot
 * @returns {Record<string, unknown> | null}
 */
function resolveManifest(productRoot) {
  // 1. Manifest nel product repo (root o sotto cruscotto.frontend/)
  const local =
    tryReadJson(join(productRoot, "product.manifest.json"))
    ?? tryReadJson(join(productRoot, "cruscotto.frontend", "product.manifest.json"))
    ?? tryReadJson(join(productRoot, "cruscotto", "product.manifest.json"));
  ...
  return tryReadJson(join(getPortalRoot(), getProjectConfig().PRJ_PRODUCT_MANIFEST));
}
