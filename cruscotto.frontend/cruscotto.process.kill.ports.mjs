...
/** @type {number} */
let windowsNetstatCacheAt = 0;

const WINDOWS_NETSTAT_CACHE_MS = 2500;
const WINDOWS_NETSTAT_TIMEOUT  = 12_000;

/**
 * Una sola esecuzione netstat -ano per refresh multi-porta (tab Process).
 *
 * @returns {string}
 */
function readWindowsNetstatOutput() {
  const now = Date.now();

  if (windowsNetstatCache != null && now - windowsNetstatCacheAt < WINDOWS_NETSTAT_CACHE_MS) {
    return windowsNetstatCache;
  }

  const result = spawnSync("netstat", ["-ano"], {
    encoding : "utf8"
  , timeout  : WINDOWS_NETSTAT_TIMEOUT
  });

  windowsNetstatCache   = result.status === 0 && result.stdout ? result.stdout : "";
  windowsNetstatCacheAt = now;

  return windowsNetstatCache;
}

/** Invalida cache netstat (dopo kill porte). */
export function clearListeningPidsCache() {
  windowsNetstatCache   = null;
  windowsNetstatCacheAt = 0;
}
 ...
 */
/**
 * PID in ascolto per più porte — una sola netstat su Windows.
 *
 * @param {number[]} ports
 * @returns {Map<number, number[]>}
 */
export function findListeningPidsByPorts(ports) {
  ...
  if (process.platform === "win32") {
    const stdout = readWindowsNetstatOutput();

    for (const port of unique) {
      out.set(port, stdout ? parseWindowsNetstatPids(stdout, port) : []);
    }
  ...
  for (const port of unique) {
    ...
    out.set(
      port
    , result.stdout ? parseUnixLsofPids(result.stdout) : []
    );
  }
